...
const App = () => {
    ...
    const [selectId, setSelectId] = useState(''); // 현재 선택한 문제 id 값 ex 144148
    ...
    const onClickRelatedProblemBtn = useCallback((i) => { // 유사 문제 로드 이벤트 
        dispatch({
            type :  LOAD_SIMILAR_PROBLEMS_REQUEST,
        });
        setSelectId(i);
        setActivesimilar(true);
    }, [selectId, activesimilar, post.mainProblemLists]);
    ...
    const onClickSwitchProblemBtn = useCallback((i) => { // 문제 교체 이벤트
        ...
        setSelectId(mainProblemIndex);
    }, [selectId, post.similarProblemLists, post.mainProblemLists].unitName);
